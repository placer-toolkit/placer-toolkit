...
export class PcDropdown extends PlacerElement {
    ...
    private getItems(includeDisabled = false): PcDropdownItem[] {
        const items: PcDropdownItem[] = [];
        const slottedElements = this.defaultSlot.assignedElements({
            flatten: true,
        });

        const findItems = (elements: Element[]) => {
            for (const element of elements) {
                if (element.localName === "pc-dropdown-item") {
                    items.push(element as PcDropdownItem);
                } else if (element.children.length > 0) {
                    findItems(Array.from(element.children));
                }
            }
        };

        findItems(slottedElements);
    ...
    private getSubmenuItems(
        ...
        const findDeepItems = (elements: Element[]) => {
            for (const element of elements) {
                if (element.localName === "pc-dropdown-item") {
                    items.push(element as PcDropdownItem);
                } else {
                    findDeepItems(Array.from(element.children));
                }
            }
        };

        findDeepItems(submenuSlot.assignedElements({ flatten: true }));
    ...
    private async showMenu() {
        ...
        const animationPromise = animateTo(this.menu, keyframes, options);
    ...
    private handleDocumentKeyDown = async (event: KeyboardEvent) => {
        ...
        if (isInSubmenu) {
            items = this.getSubmenuItems(currentSubmenuItem, true);
    ...
    private static handleSubmenuSlotChange(event: Event) {
        ...
        const hasCheckboxItems = items.some(
            (item) => item.type === "checkbox" || item.type === "radio",
        );
    ...
    private makeSelection(item: PcDropdownItem) {
        ...
        if (item.type === "radio") {
            ...
            if (item.slot === "submenu") {
                container = item.parentElement as HTMLElement;
            } else {
                let ancestor: HTMLElement | null = item.parentElement;

                while (ancestor && ancestor !== this.menu) {
            ...
            const radioItems = Array.from(
                container.querySelectorAll<PcDropdownItem>(
                    'pc-dropdown-item[type="radio"]',
                ),
            );

            radioItems.forEach((i) => (i.checked = i === item));
        }
    ...
    private async syncARIAAttributes() {
        ...
        this.menu.setAttribute("aria-expanded", this.open ? "true" : "false");
    }
