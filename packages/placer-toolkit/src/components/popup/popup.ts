...
export class PcPopup extends PlacerElement {
    ...
    /** Forces the browser to reinitialise the popup’s stacking context. */
    updateStackingContext() {
        if (!this.active || !supportsPopover) {
            return;
        }

        this.popup.hidePopover?.();

        const popoverType = this.popup.getAttribute("popover");
        ...
        if (popoverType) {
            this.popup.setAttribute("popover", popoverType);
            this.popup.showPopover?.();
        }
