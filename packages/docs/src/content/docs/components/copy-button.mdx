...
```demo:open
<pc-copy-button value="Hello World!"></pc-copy-button>
```
...
<pc-copy-button
    ...
    error-label="Whoops, didn’t work for some reason."
></pc-copy-button>
```

### Custom icons

Use the `copy-icon`, `success-icon` and `error-icon` slots to customise the icons that get displayed for each state. You can use [`<pc-icon>`](/docs/components/icon) or your own images.
...
<span class="pc-align-items-center pc-gap-s">
    <span id="my-phone">+1 555 555 0123</span>
...
<span class="pc-align-items-center pc-gap-s">
    <pc-input
        ...
        id="my-input"
        value="User input"
        style="display: inline-block; max-inline-size: 300px"
    ></pc-input>
    <pc-copy-button from="my-input.value"></pc-copy-button>
</span>
...
```demo
<pc-copy-button value="I’m awesome!" class="awesome-copy-button">
    <pc-icon
        library="default"
        icon-style="solid"
        name="clipboard"
...
<style>
    pc-copy-button.awesome-copy-button::part(button) {
    ...
    @media (hover: hover) {
        pc-copy-button.awesome-copy-button::part(button):hover {
    ...
    pc-copy-button.awesome-copy-button::part(button):active {
        background-color: color-mix(
            in oklab,
            var(--pc-color-warning-border-normal),
            var(--pc-color-mix-hover)
