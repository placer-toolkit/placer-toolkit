...
```demo
<form class="lazy-loading-options pc-stack" id="lazy-options-example">
...
<script>
    ...
    function handleLazySubmit(event) {
        ...
        for (const key of formData.keys()) {
            const value = formData.getAll(key).length > 1
...
<style>
    ...
    .lazy-loading-options pc-select {
        inline-size: 100%;
    }
</style>
```
