...
You can listen for standard events such as `click`, `mouseover`, `mouseout`, etc. as you normally would. In addition, some components have their own custom events. For example, you might listen to `pc-after-show` to determine when a dialog or drawer has opened and finished its animation.
...
This time, we receive `true`, which means the boolean attribute is now present!
