...
To properly scaffold a new component, you must follow a series of steps. These instructions are intended to guide you through the process and ensure your component is integrated correctly.

The first step is to create the component files. Inside the `packages/placer-toolkit/src/components/` directory, create a new folder named after your component using kebab‐case. Within this folder, you must create three essential files: a main entry file, a component logic file and a CSS file. Remember to add JSDoc to your component code to ensure that API documentation tables can be auto‐generated properly. Refer to existing components to maintain consistent coding conventions.

Once the files are created, you must add the component export to the codebase. Open `packages/placer-toolkit/src/placer.ts` and add an export for the new component under the **Components** section. It is essential to ensure that the new entry is alphabetically sorted to maintain order.

```typescript fileName="packages/placer-toolkit/src/placer.ts"
 export { default as PcCheckbox } from "./components/checkbox/checkbox.js";
 export { default as PcColorPicker } from "./components/color-picker/color-picker.js";
...
Finally, you can proceed with documenting the component. Create a new MDX file for your component in `packages/docs/src/content/docs/components`, following the structure of documentation files for other components.
...
The docs are powered by [Astro](https://astro.build). Check out the `packages/docs/src/content/docs/components/*.mdx` to get an idea of how pages are structured and formatted. If you’re creating a new component, it may help to use an existing component’s MDX file as a template.
...
Code demos are used to show demonstrations of different states of a component. To add one, use the fenced code block syntax with the language set to `demo`. All code demos must use HTML, no other language is accepted. CSS and JavaScript can be embedded in the code demo with `<style>` and `<script>` tags.
...
Please do not make any changes to `prettier.config.mjs` without consulting the maintainers.
