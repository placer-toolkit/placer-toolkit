...
Components such as [Input](/components/input), [Select](/components/select), [Textarea](/components/textarea) and [Checkbox](/components/checkbox) share a number of styles to give your forms a cohesive appearance. Placer Toolkit defines custom properties for these styles using the `--pc-form-control-{style}` format.
...
Panels consist of components with larger, contained surface areas like [callouts](/components/callout), [cards](/components/card), [details](/components/details) and [dialogs](/components/dialog).
...
Tooltip styles are used in the [Tooltip](/components/tooltip) component.
