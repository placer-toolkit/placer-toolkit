...
- `pre-1.0-deploy`: This branch contains a historical archive of the Placer Toolkit v0.5.1 version. You can find the historical snapshot on [v0-placer-toolkit.netlify.app](https://v0-placer-toolkit.netlify.app).
...
The docs are powered by [Astro](https://astro.build). Check out the `docs/src/content/components/*.mdx` to get an idea of how pages are structured and formatted. If you’re creating a new component, it may help to use an existing component’s MDX file as a template.
...
This convention avoids the problem of browsers lowercasing attributes, causing some frameworks to be unable to listen to them. This problem isn’t specific to one framework, but [Vue’s documentation](https://vuejs.org/guide/components/events) provides a good explanation of the problem.
