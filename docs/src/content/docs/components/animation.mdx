...
To animate an element, wrap it in a `<pc-animation>` element and set an animation name. The animation will not start until you add the `play` attribute. Refer to the [properties table](#properties) for a list of all animation options.
