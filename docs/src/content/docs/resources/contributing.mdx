...
<pc-tab-group>
    ...
    <pc-tab-panel name="yarn">
        ...
        git clone https://github.com/placer-toolkit/placer-toolkit
        cd placer-toolkit # Navigates into your clone
        yarn install # Installs dependencies for Placer Toolkit
        cd docs
        yarn install # Installs dependencies for Placer Toolkit’s docs
        ```
    </pc-tab-panel>
    <pc-tab-panel name="pnpm">
        ...
        git clone https://github.com/placer-toolkit/placer-toolkit
        cd placer-toolkit # Navigates into your clone
        pnpm install # Installs dependencies for Placer Toolkit
        cd docs
        pnpm install # Installs dependencies for Placer Toolkit’s docs
        ```
    </pc-tab-panel>
    <pc-tab-panel name="bun">
        ...
        git clone https://github.com/placer-toolkit/placer-toolkit
        cd placer-toolkit # Navigates into your clone
        bun install # Installs dependencies for Placer Toolkit
        cd docs
        bun install # Installs dependencies for Placer Toolkit’s docs
        ```
    </pc-tab-panel>
</pc-tab-group>
...
<pc-tab-group>
    ...
    <pc-tab-panel name="npm">
        ...
        npm run dev -- --host # If you want to expose the server on your network
        ```
    </pc-tab-panel>
    <pc-tab-panel name="yarn">
        ```bash
        yarn dev # Spins up the dev server provided by Astro
        yarn dev --host # If you want to expose the server on your network
        ```
    </pc-tab-panel>
    <pc-tab-panel name="pnpm">
        ```bash
        pnpm dev # Spins up the dev server provided by Astro
        pnpm dev --host # If you want to expose the server on your network
        ```
    </pc-tab-panel>
    <pc-tab-panel name="bun">
        ```bash
        bun run dev # Spins up the dev server provided by Astro
        bun run dev --host # If you want to expose the server on your network
        ```
    </pc-tab-panel>
</pc-tab-group>
...
You can also append attributes to expand the code by default or remove the CodePen button. The order of these attributes doesn’t matter.
...
To link to a GitHub issue, PR or discussion, you can use this custom Markdown syntax. This automatically generates a link to the corresponding item in the [`placer-toolkit/placer-toolkit` GitHub repository](https://github.com/placer-toolkit/placer-toolkit).
...
Components must only emit custom events, and all custom events must start with `pc-` as a namespace. For compatibility with frameworks that utilise DOM templates, custom events must have lowercase, kebab‐case names. For example, use `pc-show` instead of `pcShow`.
...
When change events are emitted by Placer Toolkit components, they should utilise the native event `change` and they should only be emitted as as result of user input. Programmatic changes, such as setting `element.value = "…"` _should not_ result in a change event being emitted. This is consistent with how native form controls work.

### Data attribute invokers

Some components can be controlled using [data attributes](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Global_attributes/data-*) that trigger specific behaviours. These components must use the following convention:

```html
<pc-button data-component="action id">Data attribute invoker</pc-button>
```

The `data-component` attribute corresponds to the component’s name without the `pc-` prefix. For example, `data-dialog` must control a `<pc-dialog>` component.

The `action` parameter is required and must be a concise descriptive term indicating the intended behaviour, e.g. `open` and `close`.

The `id` parameter must point to the `id` of the target component. You can omit the `id` only if the component wraps the element with the `data-` attribute.

```html
<pc-dialog id="my-dialog">Dialog content</pc-dialog>

<pc-button data-component="open my-dialog">
    Data attribute invoker
</pc-button>
```
...
- All form controls must extend from `PlacerFormAssociatedElement`.
- All form controls must use `name`, `value` and `disabled` properties in the same manner as `HTMLInputElement`.
- All form controls with the `disabled` property must not reflect the `disabled` attribute.
- All form controls must have an `invalid` property that reflects their validity.
- All form controls should mirror their native validation attributes such as `required`, `pattern`, `minlength`, `maxlength`, etc. when possible with the help of the `MirrorValidator`.
- All form controls must be tested to work with the standard `<form>` element.
- All form controls that do not have an editable value such as a button only need this property:
    ```typescript
    @property({ reflect: true }) value = "";
    ```
- All form controls that do have an editable value such as an input or textarea should have these properties:
    ```typescript
    @property({ attribute: false }) value = "";
    @property({ attribute: "value", reflect: true }) defaultValue = "";
    ```
    We do this to align with how native form controls work.
- All form controls that do have an editable property such as `checked` or `selected` should also have a `defaultSelected` and `defaultChecked` property respectively for use when the form is reset.
