...
Du kannst Standard‐Events wie `click`, `mouseover`, `mouseout` usw. wie gewohnt abfangen, wie du es normalerweise tun würdest. Es ist jedoch wichtig zu beachten, dass viele Events, die innerhalb des Shadow Roots einer Komponente ausgelöst werden, auf das Host‐Element [re‐targeted](https://dom.spec.whatwg.org/#retarget) werden. Dies kann dazu führen, dass z. B. mehrere `click`‐Handler ausgeführt werden, selbst wenn der/die Nutzer:in nur einmal klickt. Zudem zeigt `event.target` auf das Host‐Element, was die Sache noch verwirrender macht.

Daher solltest du fast immer Custom Events abfangen. Anstatt z. B. das `click`‐Event zu nutzen, um zu erkennen, wann ein `<pc-checkbox>` umgeschaltet wird, nutzt du das `pc-change`‐Event.
