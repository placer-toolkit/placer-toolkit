...
Wenn du neu bei Custom Elements bist, die oft als „Webkomponenten“ bezeichnet werden, wird dich dieser Abschnitt mit deren Nutzung vertraut machen.
...
In seltenen Fällen kann eine Eigenschaft ein Array, ein Objekt oder eine Funktion erfordern. Um beispielsweise die Liste vordefinierter Farbfelder des Color Pickers anzupassen, kannst du die `swatches`‐Eigenschaft auf ein Array von Farben setzen. Dies muss mit JavaScript erfolgen.
...
Du kannst Standard‐Events wie `click`, `mouseover`, `mouseout` usw. wie gewohnt abfangen. Außerdem verfügen einige Komponenten über eigene Custom Events. Beispielsweise kannst du auf das `pc-after-show`‐Event abfangen, um zu erkennen, wann ein Dialog oder Drawer geöffnet wurde und seine Animation abgeschlossen ist.
...
Einige Komponenten haben auch benannte Slots. Ein benannter Slot kann durch Hinzufügen eines Kindelements mit dem entsprechenden `slot`‐Attribut gefüllt werden. Siehst du, wie das Icon unten das Attribut `slot="prefix"` hat? Dies teilt der Komponente mit, das Icon in ihren `prefix`‐Slot zu setzen.
...
Zum Beispiel haben `<button>` und `<pc-button>` beide ein `type`‐Attribut, aber das native Element verwendet standardmäßig `submit`, während das Placer Toolkit‐Element standardmäßig `button` verwendet, da dies für die meisten Nutzer:innen die sinnvollere Standardeinstellung ist.
...
<pc-callout appearance="warning">
    <pc-icon
        ...
        slot="icon"
    ></pc-icon>
    <strong>Verlass dich nicht auf Vermutungen bezüglich der API einer Komponente!</strong><br />
...
Webkomponenten werden mit JavaScript registriert, daher kann es je nach Art und Zeitpunkt des Ladens von Placer Toolkit zu einem Flash of Undefined Custom Elements (FOUCE) kommen, wenn die Seite geladen wird. Es gibt mehrere Möglichkeiten, dies zu verhindern – mit einem Opt‐In Utility, einer benutzerdefinierten CSS‐Regel oder einem JS‐basierten Promise.
...
Placer Toolkit‐Komponenten werden mit [Lit](https://lit.dev) erstellt, einer Bibliothek, die das Erstellen von Custom Elements einfacher, wartbarer und deutlich spaßiger macht als Vanilla‐Webkomponenten. Hier sind einige nützliche Hinweise zum Rendering und zur Aktualisierung, die du kennen solltest.
...
Dieses Mal erhalten wir `true`, was bedeutet, dass das Boolean‐Attribut jetzt vorhanden ist!
