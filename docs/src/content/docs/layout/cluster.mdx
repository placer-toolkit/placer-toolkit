...
By default, items are centred in the block direction of the `pc-cluster` container. You can add any of the following [`pc-align-items-*`](/docs/layout/align-items) classes to an element with `pc-cluster` to specify how items are aligned in the block direction:
...
By default, the gap between cluster items uses `--pc-spacing-m` from your theme. You can add any of the following [`pc-gap-*`](/docs/layout/gap) classes to an element with `pc-cluster` to specify the gap between items.
