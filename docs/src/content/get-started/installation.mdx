...
<pc-tab-group>
    ...
    <pc-tab-panel name="standard-loader">
        The standard loader registers all web components in Placer Toolkit up front. If you’re only using a handful of components though, it will be much more efficient to stick with the autoloader. Alternatively, you can also [cherry pick](#cherry-picking) components if you only want to import specific ones up front. Remember to add the `pc-cloak` class to the root HTML element to apply the FOUCE patch globally.
