...
By default, items are centred in the block direction of the `pc-flank` container. You can add any of the following [`pc-align-items-*`](/layout/align-items) classes to an element with `pc-flank` to specify how items are aligned in the block direction:
...
By default, the gap between flanks uses `--pc-spacing-m` from your theme. You can add any of the following [`pc-gap-*`](/layout/gap) classes to an element with `pc-flank` to specify the gap between items.
