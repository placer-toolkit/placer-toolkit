...
By default, items stretch to fill the inline size of the `pc-stack` container. You can add any of the following [`pc-align-items-*`](/layout/align-items) classes to an element with `pc-stack` to specify how items are aligned in the inline direction:
...
By default, the gap between stack items uses `--pc-spacing-m` from your theme. You can add any of the following [`pc-gap-*`](/layout/gap) classes to an element with `pc-cluster` to specify the gap between items.
