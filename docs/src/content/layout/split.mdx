...
By default, items are centred on the cross axis of the `pc-split` container. You can add any of the following [`pc-align-items-*`](/layout/align-items) classes to an element with `pc-split` to specify how items are aligned:
...
A split’s gap determines how close items can be before they wrap. By default, the gap between split items uses `--pc-spacing-m` from your theme. You can add any of the following [`pc-gap-*`](/layout/gap) classes to an element with `pc-split` to specify the gap between items.
